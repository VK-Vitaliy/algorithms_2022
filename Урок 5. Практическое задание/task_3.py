...
"""
ВЫВОДЫ:
1) операции append, extend списка и дека у небольних массивов показывают примерно равные результаты, 
но при увеличении массива дека начинает уступать списку, метод pop показывает одинаковые результаты.
2) дека показывает очень хорошие результаты с операциями appendleft, popleft, extendleft и работает значительно быстрее.
3) получение элемента выполняется немного быстрее в обычном списке 
"""
